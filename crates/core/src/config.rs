...
impl CrossBuildConfig {
    pub fn from_environment() -> Result<Self, CrossBuildError> {
        ...
        // Inherit relevant CARGO_* env vars
        for (key, value) in env::vars() {
            if key.starts_with("CARGO_") && !value.trim().is_empty() {
                config.extra_env.insert(key, value);
            }
        }
