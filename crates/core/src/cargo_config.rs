...
mod tests {
    ...
    fn generates_basic_config() {
        ...
        let config = CargoConfigGenerator::new(target, PathBuf::from("/tmp"))
            ...
            .with_runner("qemu-x86_64")
            .with_rustflags(vec!["-C".into(), "linker=clang".into()])
            .build();
    ...
    #[test]
    fn creates_cross_config() {
        let target = crate::model::TargetTriple::parse("x86_64-unknown-linux-gnu").unwrap();
        let config = create_cross_config(
            &target,
            Some(std::path::PathBuf::from("/usr/bin/ld.lld")),
            Some(std::path::PathBuf::from("/opt/sysroot")),
