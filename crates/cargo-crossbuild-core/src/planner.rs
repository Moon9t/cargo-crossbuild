//! Build planner - resolves build requests into executable plans.

use std::collections::BTreeMap;
use std::path::{Path, PathBuf};

use crate::cache::{CacheManager, CachePolicy};
use crate::cargo_config::CargoConfigGenerator;
use crate::config::CrossBuildConfig;
use crate::error::CrossBuildError;
use crate::model::{
    BuildPlan, BuildRequest, CommandLine, ExecutionMode, HostInfo, PlanStep,
    Profile, TargetInfo, TargetTriple,
};
use crate::platform::{assess_target, detect_host};
use crate::registry::ProviderRegistry as Registry;
...
impl Planner {
    ...
    pub fn plan(
        ...
        env.insert(
            ...
            config.target_dir_for(&workspace_root).to_string_lossy().into_owned(),
        );
