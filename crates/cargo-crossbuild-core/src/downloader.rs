...
impl Downloader {
    ...
    pub fn download(&mut self, request: DownloadRequest) -> Result<DownloadResult, CrossBuildError> {
            ...
            if let Ok(checksum) = self.compute_checksum(&request.destination, request.checksum_algorithm) {
                if checksum == request.expected_checksum.as_deref().unwrap_or("") {
                    return Ok(DownloadResult {
                        ...
                        size_bytes: fs::metadata(&request.destination)
                            .map(|m| m.len())
                            .unwrap_or(0),
                        checksum,
        ...
        // Verify checksum - need to re-read the file for verification
        if let Some(expected) = &request.expected_checksum {
            ...
            let actual = self.compute_checksum(&temp_path, request.checksum_algorithm)?;
            if actual != *expected {
        ...
        // Compute final checksum if needed
        let final_checksum = if request.expected_checksum.is_some() {
            ...
            // We need to recompute it - read the file again
            self.compute_checksum(&request.destination, request.checksum_algorithm)?
        };
...
trait ChecksumHasher: Send {
    ...
    fn finalize_boxed(self: Box<Self>) -> String;
}
