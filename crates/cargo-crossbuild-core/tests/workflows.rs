...
fn cache_policy_derives_stable_keys() {
    ...
    // On Windows, the cache key replaces `:` and path separators with `_`
    // C:/workspace -> C__workspace (colon and slash both become _)
    assert_eq!(policy.cache_key(&workspace_root, &target), "C__workspace::x86_64-unknown-linux-gnu");
}
...
fn download_request_records_provenance() {
    ...
    // The provenance label includes the checksum algorithm prefix
    assert_eq!(
...
fn release_and_install_helpers_are_deterministic() {
    ...
    let target = TargetTriple::parse("x86_64-unknown-linux-gnu").unwrap();
    // Use with_release_mode(true) to require release mode
    let validation = ValidationPlan::new("release-validation", target).with_release_mode(true);
    ...
    // "release" label with with_release_mode(true) requires release mode
